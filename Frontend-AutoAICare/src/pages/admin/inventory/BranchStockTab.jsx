...
const BranchStockTab = () => {
    ...
    }, [branchFilter, stockStatusFilter, debouncedSearchTerm, pageSize]);

    useEffect(() => {
        fetchBranches();
        fetchBranchStocks();
        fetchSummary();
    }, [branchFilter, stockStatusFilter, partFilter, debouncedSearchTerm, pagination.page, pageSize, getCurrentBranchId()]);

    const fetchBranches = async () => {
        try {
            const response = await api.get('/branches/');
            let branchesData = response.data.results || response.data;

            // If branch admin, only show their branch
            if (isBranchAdmin) {
                branchesData = branchesData.filter(b => b.id === user.branch);
            }
    ...
    const fetchSummary = async () => {
        try {
            ...
            const response = await api.get(`/jobcards/branch-stock/low_stock_alert/?${params}`);
            setSummary({
                low_stock_count: response.data.length,
                total_branches: isBranchAdmin ? 1 : branches.length
            });
        } catch (error) {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* <div className="flex justify-end">
                <button
                    onClick={() => navigate('/admin/inventory?tab=transfers&action=create')}
                                    ...
                                    <tr key={stock.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <button
                                                    onClick={() => navigate(`/admin/inventory?tab=transfers&action=create&part=${stock.part}&from_branch=${stock.branch}`)}
                    ...
                    <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                        <div className="flex items-center justify-between">
                            <p className="text-sm text-gray-600">
                                Showing page {pagination.page} of {pagination.totalPages} ({pagination.count} total records)
                            </p>
                            <div className="flex gap-2">
                                <button
                                    onClick={() => setPagination({ ...pagination, page: pagination.page - 1 })}
                                    disabled={!pagination.hasPrevious}
                                    className="px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm"
                                >
                                    Previous
                                </button>
                                <button
