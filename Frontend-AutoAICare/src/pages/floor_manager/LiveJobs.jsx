...
const LiveJobs = () => {
  ...
  const handleJobClick = (jobId) => {
    ...
    navigate(`/floor-manager/job/${jobId}`);
  };

  const fetchActiveJobs = useCallback(async () => {
    try {
      setLoading(true);
      const response = await api.get('/jobcards/floor-manager/jobs/', {
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900 truncate">{job.supervisor_details?.name || 'Unassigned'}</p>
                </div>
                <div>
    ...
    <div className="w-full overflow-x-auto">
      <div className="flex gap-4 pb-4 min-w-max">
        {STATUS_COLUMNS.map((column) => {
          ...
          return (
            <div key={column.key} className="flex flex-col w-72 md:w-80 lg:w-96 xl:w-[22rem] flex-shrink-0">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="space-y-1 pt-1 border-t border-gray-100 italic text-gray-500">
                          {job.supervisor_details?.name && (
                            <div className="flex items-center gap-1.5 text-[10px] truncate"><User size={10} />Supervisor : {job.supervisor_details?.name || 'Unassigned'}</div>
                          )}
                          ...
                          <div className="flex items-center gap-1.5 text-[10px] truncate"><Wrench size={10} /> {job.booking_details?.package_details?.name}</div>
                        </div>
                        {job.job_started_at && (
                        ...
                        {/* Reward Preview */}
                        {job.potential_reward && (
                          <div className="mt-2 pt-2 border-t border-gray-100">
                            <div className={`flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-semibold ${job.potential_reward.transaction_type === 'reward'
                              ? 'bg-green-50 text-green-700 border border-green-200'
                              : 'bg-red-50 text-red-700 border border-red-200'
                              }`}>
                              <span>{job.potential_reward.transaction_type === 'reward' ? '+' : '-'}₹{job.potential_reward.total_amount.toFixed(0)}</span>
                              {job.potential_reward.tier && (
                                <span className="text-[8px] opacity-75">({job.potential_reward.tier.replace('tier_', 'T')})</span>
                              )}
                            </div>
                          </div>
                        )}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Live Jobs</h1>
          <p className="text-sm sm:text-base text-gray-600 mt-1">Real-time status of all active job cards</p>
        </div>
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div className="flex bg-gray-100 p-1 rounded-lg border border-gray-200">
            <Button
              ...
              className={`flex items-center gap-2 h-8 ${viewMode === 'board' ? 'shadow-sm' : ''}`}
            >
              <LayoutGrid size={14} />
            ...
            <Button
              ...
              className={`flex items-center gap-2 h-8 ${viewMode === 'list' ? 'shadow-sm' : ''}`}
            >
              <List size={14} />
