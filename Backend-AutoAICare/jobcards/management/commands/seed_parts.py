...
class Command(BaseCommand):
    help = 'Seed realistic parts data for car detailing services'
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            required=True,
            help='Company ID to seed parts for (required - parts are company-specific)',
        )
    ...
    def handle(self, *args, **options):
        ...
        if not company_id:
            self.stdout.write(self.style.ERROR("Error: --company argument is required"))
            self.stdout.write("Usage: python manage.py seed_parts --company=<company_id>")
            return
        
        try:
            company = Company.objects.get(id=company_id)
            self.stdout.write(self.style.SUCCESS(f"Seeding parts for company: {company.name}"))
            self.stdout.write(self.style.WARNING(f"Note: 'is_global=True' means available to ALL branches within {company.name}"))
        ...
        for part_data in parts_data:
            # Skip placeholders or invalid entries if any
            if not isinstance(part_data, dict) or 'sku' not in part_data:
                continue
            
            # Set company for this part
            part_data['company'] = company
            
            # is_global=True means available to all branches within THIS company
            # is_global=False means available to specific branch only (would need branch specified)
            # For seed data, we keep is_global=True so parts are available to all branches of the company
            # Note: is_global does NOT mean global across all companies - that's prevented by company field
            
            # Check if part exists by SKU for this company
            existing_part = Part.objects.all_companies().filter(
                sku=part_data['sku'],
                company=company
            ).first()
            ...
            if existing_part:
                ...
                existing_part.save()
                updated_count += 1
                self.stdout.write(f"Updated: {part_data['name']} ({part_data['sku']})")
            else:
                # Create new part
                Part.objects.create(**part_data)
                created_count += 1
                self.stdout.write(self.style.SUCCESS(f"Created: {part_data['name']} ({part_data['sku']})"))

        self.stdout.write(self.style.SUCCESS(f"\n✅ Seed complete!"))
        self.stdout.write(f"   Created: {created_count} parts")
        self.stdout.write(f"   Updated: {updated_count} parts")
        
        total_parts = Part.objects.all_companies().filter(company=company).count()
        self.stdout.write(f"   Total: {total_parts} parts in database for this company")
